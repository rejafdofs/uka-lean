...
structure Responsum where
  /-- 狀態符號 -/
  status  : StatusCodis
  /-- Value 頭部（SakuraScript）にゃん。GET の應答に入れるにゃ -/
...
def adProtocollum (r : Responsum) : String :=
  let statusLinea := r.status.lineaStatus ++ crlf
  let charset := "Charset: UTF-8" ++ crlf
  let valor := match r.valor with
    | some v => s!"Value: {v}" ++ crlf
    | none => ""
  let extra := r.cappitta.foldl
    (fun acc (k, v) => acc ++ s!"{k}: {v}" ++ crlf) ""
  statusLinea ++ charset ++ valor ++ extra ++ crlf
